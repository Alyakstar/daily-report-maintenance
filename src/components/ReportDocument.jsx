import { LINES } from "../constants";
import { formatReportDate } from "../utils/format";

const SOURCE_COLUMNS = ["BM", "PM", "IMP"];

function TextWithImage({ text, image, alt }) {
  return (
    <div className="report-combined-cell">
      <div className="report-cell-text">
        {text || "-"}
      </div>

      {image && (
        <img
          className="report-cell-image"
          src={image}
          alt={alt}
        />
      )}
    </div>
  );
}

export default function ReportDocument({
  reports = [],
  reportRef,
}) {
  const summary = Object.fromEntries(
    LINES.map((line) => [line, 0])
  );

  reports.forEach((item) => {
    if (summary[item.line] !== undefined) {
      summary[item.line] +=
        Number(item.lineStop) || 0;
    }
  });

  const first = reports[0] || {};

  const reportDate = first.date;
  const teamShift = first.teamShift || "White";
  const workShift =
    first.workShift || "Night Shift";

  return (
    <div
      className="report-paper exact-report"
      ref={reportRef}
    >
      {/* HEADER */}

      <section className="exact-top-header">
        <div className="exact-company">
          <strong>PT TMMIN</strong>
          <span>EPSD SUNTER II</span>
          <span>Maintenance Department</span>
        </div>

        <div className="exact-title-block">
          <h2>DAILY REPORT PROBLEM</h2>

          <p>
            {formatReportDate(reportDate)} (
            {" "}
            {teamShift}
            {" "}
            ) {workShift}
          </p>
        </div>
      </section>

      <div className="yellow-strip" />

      {/* SUMMARY */}

      <section className="exact-summary-strip">
        {LINES.map((line) => (
          <div
            className="exact-summary-box"
            key={line}
          >
            <span>
              {line.toUpperCase()}
            </span>

            <strong>
              {summary[line]}'
            </strong>
          </div>
        ))}
      </section>

      {/* TABLE */}

      <table className="exact-report-table">
        <thead>
          <tr className="main-head-row">
            <th
              rowSpan="2"
              className="col-no"
            >
              NO
            </th>

            <th
              rowSpan="2"
              className="col-line"
            >
              LINE
            </th>

            <th
              rowSpan="2"
              className="col-machine"
            >
              MESIN
            </th>

            <th
              colSpan="3"
              className="source-group"
            >
              SOURCE
            </th>

            <th
              rowSpan="2"
              className="col-stop"
            >
              LINE
              <br />
              STOP
            </th>

            <th
              rowSpan="2"
              className="col-frequency"
            >
              FREQUENCY
            </th>

            <th
              rowSpan="2"
              className="col-problem"
            >
              PROBLEM
            </th>

            <th
              rowSpan="2"
              className="col-root"
            >
              ROOTCAUSE
            </th>

            <th
              rowSpan="2"
              className="col-action"
            >
              ACTION
            </th>

            <th
              rowSpan="2"
              className="col-pic"
            >
              PIC
            </th>
          </tr>

          <tr className="source-head-row">
            <th className="source-bm">
              BM
            </th>

            <th className="source-pm">
              PM
            </th>

            <th className="source-imp">
              IMP
            </th>
          </tr>
        </thead>

        <tbody>
          {reports.map((item, index) => (
            <tr key={item.id || index}>
              {/* NO */}

              <td className="center">
                {index + 1}
              </td>

              {/* LINE */}

              <td>
                {item.line}
              </td>

              {/* MACHINE */}

              <td>
                {item.machine}
              </td>

              {/* SOURCE */}

              {SOURCE_COLUMNS.map(
                (source) => (
                  <td
                    className="center source-check"
                    key={source}
                  >
                    {item.source === source
                      ? "✓"
                      : ""}
                  </td>
                )
              )}

              {/* LINE STOP */}

              <td className="center exact-line-stop">
                {Number(item.lineStop) || 0}'
              </td>

              {/* FREQUENCY */}

              <td className="center">
                {Number(item.frequency) || 1}x
              </td>

              {/* PROBLEM + FOTO PROBLEM */}

              <td className="combined-report-td">
                <TextWithImage
                  text={item.problem}
                  image={item.problemImage}
                  alt="Problem"
                />
              </td>

              {/* ROOTCAUSE */}

              <td>
                {item.rootcause || "-"}
              </td>

              {/* ACTION + FOTO ACTION */}

              <td className="combined-report-td">
                <TextWithImage
                  text={item.action}
                  image={item.actionImage}
                  alt="Penanggulangan"
                />
              </td>

              {/* PIC */}

              <td className="center">
                {item.pic || "-"}
              </td>
            </tr>
          ))}

          {reports.length === 0 && (
            <tr>
              <td
                colSpan="12"
                className="empty-report-row"
              >
                NO DATA
              </td>
            </tr>
          )}
        </tbody>
      </table>
    </div>
  );
}
